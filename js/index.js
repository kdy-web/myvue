...
//获取轮播图图片数量			
			var len_lunbo=$('.silde_wrap li').length
//生成小点		
			for(var i=0;i<len_lunbo-1;i++){
				$("#silde_numbtn").append("<li></li>");		
 }
//生成小点内容			
			for(var j=0;j<len_lunbo-1;j++){
...
//获取图片宽度	
			var width=$('.silde_wrap').width()/len_lunbo
//小点悬浮事件			
...
//获取下标			
       ...
       $(this).addClass('active').siblings().removeClass('active');
		$('.index_topBox .silde_bg li').eq(slide_n).css("display","block").siblings().css("display","none");		
			})
...
//移动函数
			function move(){
				...
				if(slide_n==len_lunbo){
				slide_n=1
					$('.silde_wrap').css({left:0});
					
				}else if(slide_n==-1){
					$('.silde_wrap').css({left:-(slide_n-1)*width+"px"});
					slide_n=len_lunbo-2
				}
					$('.silde_wrap').stop().animate({left:-slide_n*width+"px"},1000);
				
             
				

				  if(slide_n==len_lunbo-1){
 ...
 //根据下标，背景出现  
   ...
   $("#silde_numbtn li").eq(slide_n).addClass('active').siblings().removeClass('active');
  //根据下标，背景出现    
    $('.index_topBox .silde_bg li').eq(slide_n).css("display","block").siblings().css("display","none")
  }}
